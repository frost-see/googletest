...
if [ "$CLEAN" = "ON" ]; then
    if [ -d "$BUILD_DIR" ]; then
        print_warning "删除现有 build 目录: $BUILD_DIR"
        rm -rf "$BUILD_DIR"
    fi
fi
...
# 创建 build 目录
if [ ! -d "$BUILD_DIR" ]; then
    print_info "创建 build 目录: $BUILD_DIR"
    mkdir -p "$BUILD_DIR"
fi
...
if [ -d "$BUILD_DIR/lib" ]; then
    ...
    ls -lh "$BUILD_DIR/lib" 2>/dev/null || true
fi
if [ -d "$BUILD_DIR/bin" ]; then
    echo "  可执行文件位置: $BUILD_DIR/bin"
    ls -lh "$BUILD_DIR/bin" 2>/dev/null | head -5 || true
fi
echo ""
...
if [ "$BUILD_SAMPLES" = "ON" ]; then
    echo "     ./$BUILD_DIR/bin/sample1_unittest"
fi
echo ""
